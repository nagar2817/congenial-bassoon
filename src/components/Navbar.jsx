...
import logo2 from "../assets/images/movie-4.png";
...
function Navbar() {
    ...
    const { header, user,setActiveUserId,setUser} = useContext(Contextpage);
    ...
    const Navdata = [
        ...
        {
            id: 2,
            headername: "Watched Movies",
            Name: "Watched",
            link:"/watched"
        },
        {
            id: 3,
            headername: "WatchList Movies",
            Name: "WantToWatch",
            link:"/wantToWatch"
        },
        {
    ...
    const handleLogout = async ()=>{
        await auth.signOut(toast.error("Logout successfully"))
        setActiveUserId(-1)
        setUser('');
    }
            ...
            <nav className={`${activemobile ? 'block' : 'hidden'} fixed bg-black/90 md:bg-black h-full w-full md:w-[15rem] z-30 md:block`}>
                ...
                >
                    <Link to="/" className="logo flex flex-col justify-center items-center m-7 gap-2" onClick={() => setActivemobile(!activemobile)}>
                        ...
                        <h1 className="text-gray-400/70 font-bold text-2xl text-center">Movies Zone</h1>
                    </Link>
                </motion.div>
